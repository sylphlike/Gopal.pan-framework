...
import com.horse.framework.utils.enums.DateEnums;
import com.horse.framework.utils.validator.PatternPool;
import com.horse.framework.utils.validator.Regular;
import com.horse.framework.utils.validator.StrValidator;
...
public class IdCardUtil {
	...
	public static boolean isValidCard18(String idCard) {
		...
		Pattern citizenId = PatternPool.CITIZEN_ID;
		if(citizenId.matcher(idCard).matches()){
			if (idCard.length() == 18) {
				try {
					//校验生日
					if (!StrValidator.isBirthday(idCard.substring(6, 14))) {
						return false;
					}
	...
	public static boolean isValidCard15(String idCard) {
		...
		if (Regular.isMatch(PatternPool.NUMBERS, idCard)) {
			// 省份
			...
			//校验生日（两位年份，补充为19XX）
			return StrValidator.isBirthday("19" + idCard.substring(6, 12));
		} else {
			return false;
		}
	}
	...
	public static String convert15To18(String idCard) {
		...
		// 十七位数字本体码加权求和
		for (int i = 0; i < 17; i++) {
			sum = sum + Integer.parseInt(id17.substring(i, i + 1)) * wf[i];
		}
		// 计算模
		y = sum % 11;
		// 通过模得到对应的校验码 cc[y]
	...
	public static LocalDate getBirthDate(String idCard) {
		...
		return null == birthByIdCard ? null : DateUtils.string2Date(birthByIdCard, DateEnums.yyyyMMdd.getPatterns());
	}
